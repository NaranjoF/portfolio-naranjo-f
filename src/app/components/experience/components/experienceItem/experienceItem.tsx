  ...
  return (
    <div className="max-w-[20rem] md:max-w-[42rem] border border-gray-12 rounded-2xl p-10 flex flex-col justify-center items-center bg-gradient-to-b from-[#1A1A1A] from-[-10%] to-[rgba(26,26,26,0)] to-[68%] shadow-sm shadow-black">
      <div className="border border-gray-20 rounded-2xl relative">
        ...
        <Image
          src={companyLogo}
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[9rem] md:w-[13rem]"
          alt={companyLogoAlt}
        />
        ...
        <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 border border-gray-12 rounded-full px-3 py-1 md:px-5 md:py-2 bg-gray-06">
          <p className="text-md md:text-lg font-semibold">{companyName}</p>
        </div>
      </div>
      ...
      <div className="w-full flex justify-center items-center flex-col">
        <div className="w-full flex flex-col mt-10">
          <div>
            <p className="text-xl text-center md:text-left font-semibold">
              {position}
            </p>
          </div>
          <div className="w-full text-xs md:text-sm flex flex-col md:flex-row justify-center md:justify-between items-center">
            <p>{companyLocation}</p>
            <p>{date}</p>
          </div>
        </div>
